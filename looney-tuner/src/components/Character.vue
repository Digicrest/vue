        ...
        <div class="card-block">
            <div v-if="infoVisible" class="more-info" v-on:click="toggleMore()" style="margin-top: 20px;">
                ...
                <div v-for="(rel, i) in charObj.relationships" :key="i">
                    <hr>
                    <label><strong>{{ rel.name || "???" }}</strong></label>
                    <p><em>{{ rel.description }}</em></p>
                </div>
            </div>
            <div v-if="statsVisible" v-on:click="toggleStats">
                <pin v-if="pinned" />
                <character-stats :isPinned="pinned" :textColor="rarityColor" :stats="charObj.stats" :limits="statLimits" :name="charObj.name"/>
            </div>
            <div v-if="!statsVisible && !infoVisible">
                ...
                <h6 class="card-subtitle">{{ charObj.region }}</h6>

                <div id="character-img">
                    <spinnable-image v-on:togglePinned="pinned = !pinned" :sauce="imagePath()" :name="charObj.name"/>
                    <pin v-if="pinned" />
                    <br>
                ...
                <div class="icons">
                    ...
                    <button 
                        v-for="(tag, i) in charObj.tags" :key="i"
...
export default {
    ...
    methods: {
        imagePath: function() {
            return require(`../assets/characters/${this.charName.toLowerCase()}/${this.charObj.name.split(" ").join("_").split("'").join("_")}.png`);
        },
        toggleMore: function () {
    ...
    watch: {
        "pinned": function() {
            this.$emit("setPinned", { name: this.charObj.name, pin_status: this.pinned })
        }
    },
    created(){
        this.rarityIcon();
